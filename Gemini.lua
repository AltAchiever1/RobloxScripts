...
local AimAssistEnabled = false -- New Aim Assist Variable
local ESPEnabled = false
local TracersEnabled = false
local TeamCheck = true
local AimPart = "HumanoidRootPart" -- Changed to Root for "Middle of ESP" lock
local AimSmoothness = 0.5
local MaxDistance = 200 -- 200 Meters/Studs limit
...
MainFrame.Size = UDim2.new(0, 250, 0, 350) -- Adjusted height for extra toggle
...
local function UpdateESP()
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local inRange = distanceValue <= MaxDistance

            local screenPos, onScreen = Camera:WorldToViewportPoint(root.Position)

            if ESPEnabled and onScreen and inRange then
...
local function GetClosestPlayer()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild(AimPart) then
            ...
            if distFromMe > MaxDistance then continue end -- Distance filter for aimbot
...
RunService.RenderStepped:Connect(function()
    ...
    if target then
        ...
        if AimbotEnabled then
            -- Full Snap Lock
            mousemoverel(direction.X * AimSmoothness, direction.Y * AimSmoothness)
        elseif AimAssistEnabled then
            -- Heavy Assistance (Only pulls if cursor is near target)
            if direction.Magnitude < 150 then 
                mousemoverel(direction.X * 0.25, direction.Y * 0.25)
            end
        end
    end
end)
